...
ABS_TAKE_VALUES = [80.0, 120.0, 160.0, 240.0, 320.0]
ABS_STOP_VALUES = [80.0, 120.0, 160.0, 240.0]
...
def default_output_root() -> Path:
    return default_casebook_root() / "take_experiments_abs_stop"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Sweep absolute take-profit plus absolute stop-loss models over case window 0001-0005."
    )
...
def _apply_variant(base_pack: dict[str, Any], abs_take: float, abs_stop: float) -> dict[str, Any]:
    ...
    for preset_name, preset_raw in presets.items():
        ...
        meta["take_experiment"] = {
            "mode": "ABSOLUTE",
            "take_profit_value": float(abs_take),
            "stop_loss_value": float(abs_stop),
            "long_exit_mode": "disabled_except_absolute_take_or_stop_or_force_close",
        }
    ...
    meta_root["take_experiment"] = {
        ...
        "stop_values": list(ABS_STOP_VALUES),
        "long_exit_mode": "disabled_except_absolute_take_or_stop_or_force_close",
    }
    pack["meta"] = meta_root
...
def _row_sort_key(row: dict[str, Any]) -> tuple[Any, Any, Any, Any]:
    return (
        ...
        -int(row.get("selected_trades") or 0),
    )


def _build_markdown(rows: list[dict[str, Any]], output_root: Path) -> str:
    lines: list[str] = []
    lines.append("# Absolute Take + Stop Sweep For Case Window 0001-0005")
    lines.append("")
    lines.append(f"- Output root: `{output_root}`")
    lines.append(f"- ABS take values: `{', '.join(str(int(v)) for v in ABS_TAKE_VALUES)}`")
    lines.append(f"- ABS stop values: `{', '.join(str(int(v)) for v in ABS_STOP_VALUES)}`")
    lines.append("- Long Exit rules are disabled; only absolute TP, absolute SL, and force-close-at-window-end remain.")
    lines.append("")
    lines.append("| Rank | ABS Take | ABS Stop | Hit cases | Trades | Win rate % | Net profit | Profit factor | Max DD % | TP exits | SL exits |")
    lines.append("|---:|---:|---:|---|---:|---:|---:|---:|---:|---:|---:|")
    for rank, row in enumerate(rows, start=1):
        hits = ",".join(list(row.get("hit_cases") or [])) or "-"
        lines.append(
            f"| {rank} | {row['abs_take_value']} | {row['abs_stop_value']} | {hits} | {row['selected_trades']} | "
            f"{row['win_rate_pct']} | {row['net_profit']} | {row['profit_factor']} | {row['max_drawdown_pct']} | "
            f"{row['take_profit_count']} | {row['stop_loss_count']} |"
        )
    lines.append("")
    return "\n".join(lines).rstrip() + "\n"


def sweep_absolute_take_stop(
    *,
    report_json: Path,
    ...
    combos = [(take, stop) for take in ABS_TAKE_VALUES for stop in ABS_STOP_VALUES]
    for index, (abs_take, abs_stop) in enumerate(combos, start=1):
        variant_id = f"abs_take_{int(abs_take)}_stop_{int(abs_stop)}"
        print(f"[{index}/{len(combos)}] Running absolute take/stop variant: {variant_id}")
        variant_pack = _apply_variant(base_pack, abs_take, abs_stop)
        ...
        row = {
            ...
            "take_profit_count": int(portfolio.get("take_profit_count") or 0),
            "stop_loss_count": int(portfolio.get("stop_loss_count") or 0),
    ...
    summary = {
        ...
        "abs_stop_values": list(ABS_STOP_VALUES),
        "rows": ranked_rows,
        "best_variant": dict(ranked_rows[0]) if ranked_rows else None,
    }
    summary_json = output_root / "absolute_take_stop_sweep_0001_0005.json"
    summary_md = output_root / "absolute_take_stop_sweep_0001_0005.md"
