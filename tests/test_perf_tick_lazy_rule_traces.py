...
class TickLazyRuleTracePerfTests(unittest.TestCase):
    ...
    def test_tick_flat_exit_signal_does_not_build_unused_rule_trace(self) -> None:
        df = self._sample_stream()
        rules_model = {
            "Long Entry": [],
            "Long Exit": [[Rule(timeframe="1m", mode="state", field="ms", op="=", value="GREEN")]],
            "Short Entry": [],
            "Short Exit": [],
        }
        ...
        ticks = [
            ...
            {"T": int((df.index[3] + pd.Timedelta(seconds=1)).value // 1_000_000), "p": "101.6"},
        ]

        with patch("app.backtest.iter_aggtrades_futures_daily", return_value=ticks), \
             patch("app.backtest._build_tab_rule_trace", side_effect=AssertionError("unused trace should stay lazy")):
    ...
    def test_tick_open_builds_only_consumed_entry_trace(self) -> None:
        df = self._sample_stream()
        rules_model = {
        ...
        def fake_trace(tab_name, *args, **kwargs):
            trace_calls.append(tab_name)
            return f"trace:{tab_name}"

        with patch("app.backtest.iter_aggtrades_futures_daily", return_value=ticks), \
             patch("app.backtest._build_tab_rule_trace", side_effect=fake_trace):
            result = run_backtest_tick(
                symbol="BTCUSDT",
                df_1m_full=df.copy(),
                start=df.index[0],
        ...
        self.assertEqual(trace_calls, ["Long Entry"])
        self.assertEqual(result.trades[0].entry_rule_trace, "trace:Long Entry")
