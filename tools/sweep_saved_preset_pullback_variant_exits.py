...
from tools.sweep_saved_preset_pullback_variants import (
    _append_entry_rules,
    _baseline_entry_filters,
    _variant_library,
)
...
def _apply_exit_values(base_cfg: BacktestConfig, *, stop_value: float, take_value: float) -> BacktestConfig:
    ...
    next_cfg.stop_loss_mode = "PERCENT" if float(stop_value) > 0 else "OFF"
    next_cfg.stop_loss_value = float(stop_value)
    next_cfg.stop_loss_pct = float(stop_value) if float(stop_value) > 0 else 0.0
    next_cfg.stop_loss_timeframe = "1m"
    next_cfg.stop_loss_field = ""
    next_cfg.stop_loss_offset_pct = 0.0
    next_cfg.take_profit_mode = "PERCENT" if float(take_value) > 0 else "OFF"
    next_cfg.take_profit_value = float(take_value)
    next_cfg.take_profit_pct = float(take_value) if float(take_value) > 0 else 0.0
...
def _result_row(res: Any, *, stop_value: float, take_value: float) -> Dict[str, Any]:
...
def _replay_exit_combo(base_result: Any, base_cfg: BacktestConfig, *, stop_value: float, take_value: float) -> Any:
...
def _run_exit_combo_chunk(combos: List[Tuple[float, float]]) -> List[Dict[str, Any]]:
    ...
    for stop_value, take_value in list(combos or []):
        ...
        rows.append(_result_row(res, stop_value=float(stop_value), take_value=float(take_value)))
    return rows
    ...
    *,
    ...
    if resolved_workers <= 1 or len(combos_to_replay) <= 1:
        for index, (stop_value, take_value) in enumerate(combos_to_replay, start=1):
            ...
            rows.append(_result_row(res, stop_value=stop_value, take_value=take_value))
...
def main() -> int:
    ...
    variants = {name: (long_rules, short_rules) for name, long_rules, short_rules in _variant_library()}

    parser = argparse.ArgumentParser(description="Sweep percent exits for one pullback-rule variant using replay.")
    parser.add_argument("--preset", dest="preset_name", default=None)
    parser.add_argument("--variant", required=True, help="Variant name from sweep_saved_preset_pullback_variants.py")
    ...
    parser.add_argument("--stop-values", default="0,1,1.5,2,2.5,3,4,5")
    parser.add_argument("--take-values", default="3,4,5,6,7,8,10")
    ...
    long_rules, short_rules = variants[args.variant]
    ...
    entry_filters = _baseline_entry_filters()
    ...
    print("Running base backtest for the selected rule variant...", flush=True)
    ...
    rows: List[Dict[str, Any]] = [_result_row(base_result, stop_value=0.0, take_value=0.0)]
    ...
    best_result = base_result if best_combo == (0.0, 0.0) else _replay_exit_combo(
        base_result,
        base_cfg,
        stop_value=best_combo[0],
        take_value=best_combo[1],
    )
    ...
    best_cfg = _apply_exit_values(base_cfg, stop_value=best_combo[0], take_value=best_combo[1])
    bundle_dir = _write_verification_bundle(
        repo_root=repo_root,
        preset_name=f"{preset_name}_{args.variant}_best_exits",
        symbol=symbol,
        start=start,
        end=end,
        bt_mode=bt_mode,
    ...
    runs_dir = Path(args.output_dir).resolve() if args.output_dir else (repo_root / "runs" / f"{_slug(preset_name)}_{_slug(args.variant)}_exit_sweep_{pd.Timestamp.now(tz='UTC').strftime('%Y%m%d_%H%M%S')}")
